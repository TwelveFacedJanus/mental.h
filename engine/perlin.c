#include "mental.h"
#include <math.h>
#include <stdlib.h>

// Таблица перестановок для шума Перлина
static int p[512];
static int permutation[256] = {
    151,160,137,91,90,15,131,13,201,95,96,53,194,233,7,225,140,36,103,30,69,142,8,99,37,240,21,10,23,
    190,6,148,247,120,234,75,0,26,197,62,94,252,219,203,117,35,11,32,57,177,33,88,237,149,56,87,174,20,125,136,171,168,
    68,175,74,165,71,134,139,48,27,166,77,146,158,231,83,111,229,122,60,211,133,230,220,105,92,41,55,46,245,40,244,
    102,143,54,65,25,63,161,1,216,80,73,209,76,132,187,208,89,18,169,200,196,135,130,116,188,159,86,164,100,109,198,173,186,3,64,52,217,226,250,124,123,5,202,38,147,118,126,255,82,85,212,207,206,59,227,47,16,58,17,182,189,28,42,223,183,170,213,119,248,152,2,44,154,163,70,221,153,101,155,167,43,172,9,129,22,39,253,19,98,108,110,79,113,224,232,178,185,112,104,218,246,97,228,251,34,242,193,238,210,144,12,191,179,162,241,81,51,145,235,249,14,239,107,49,192,214,31,181,199,106,157,184,84,204,176,115,121,50,45,127,4,150,254,138,236,205,93,222,114,67,29,24,72,243,141,128,195,78,66,215,61,156,180
};

// Функция для инициализации таблицы перестановок
void mental_init_perlin() {
    for (int i = 0; i < 256; i++) {
        p[256 + i] = p[i] = permutation[i];
    }
}

// Функция для плавной интерполяции
static float fade(float t) {
    return t * t * t * (t * (t * 6 - 15) + 10);
}

// Функция для линейной интерполяции
static float lerp(float t, float a, float b) {
    return a + t * (b - a);
}

// Функция для вычисления градиента
static float grad(int hash, float x, float y, float z) {
    int h = hash & 15;
    float u = h < 8 ? x : y;
    float v = h < 4 ? y : h == 12 || h == 14 ? x : z;
    return ((h & 1) == 0 ? u : -u) + ((h & 2) == 0 ? v : -v);
}

// Основная функция шума Перлина
float mental_perlin_noise(float x, float y, float z) {
    int X = (int)floor(x) & 255;
    int Y = (int)floor(y) & 255;
    int Z = (int)floor(z) & 255;
    
    x -= floor(x);
    y -= floor(y);
    z -= floor(z);
    
    float u = fade(x);
    float v = fade(y);
    float w = fade(z);
    
    int A = p[X] + Y;
    int AA = p[A] + Z;
    int AB = p[A + 1] + Z;
    int B = p[X + 1] + Y;
    int BA = p[B] + Z;
    int BB = p[B + 1] + Z;
    
    return lerp(w, lerp(v, lerp(u, grad(p[AA], x, y, z),
                                   grad(p[BA], x - 1, y, z)),
                           lerp(u, grad(p[AB], x, y - 1, z),
                                   grad(p[BB], x - 1, y - 1, z))),
                   lerp(v, lerp(u, grad(p[AA + 1], x, y, z - 1),
                                   grad(p[BA + 1], x - 1, y, z - 1)),
                           lerp(u, grad(p[AB + 1], x, y - 1, z - 1),
                                   grad(p[BB + 1], x - 1, y - 1, z - 1))));
}

// Функция для создания текстуры земли на основе шума Перлина
unsigned char* mental_generate_ground_texture(int width, int height, float scale) {
    unsigned char* data = malloc(width * height * 3);
    if (!data) {
        MENTAL_DEBUG("Failed to allocate memory for ground texture");
        return NULL;
    }
    
    // Инициализируем шум Перлина
    mental_init_perlin();
    
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            // Нормализуем координаты
            float nx = (float)x / width * scale;
            float ny = (float)y / height * scale;
            
            // Генерируем несколько слоев шума для более реалистичного вида
            float noise1 = mental_perlin_noise(nx * 2.0f, ny * 2.0f, 0.0f) * 0.5f;
            float noise2 = mental_perlin_noise(nx * 4.0f, ny * 4.0f, 0.0f) * 0.25f;
            float noise3 = mental_perlin_noise(nx * 8.0f, ny * 8.0f, 0.0f) * 0.125f;
            
            float total_noise = noise1 + noise2 + noise3;
            
            // Нормализуем шум к диапазону [0, 1]
            total_noise = (total_noise + 1.0f) * 0.5f;
            
            // Создаем цвет земли на основе шума
            int index = (y * width + x) * 3;
            
            // Базовый цвет земли (коричневый)
            int base_r = 139; // Темно-коричневый
            int base_g = 69;  // Темно-коричневый
            int base_b = 19;  // Темно-коричневый
            
            // Вариации цвета на основе шума
            int variation = (int)(total_noise * 50);
            
            data[index] = (unsigned char)(base_r + variation);     // R
            data[index + 1] = (unsigned char)(base_g + variation); // G
            data[index + 2] = (unsigned char)(base_b + variation); // B
        }
    }
    
    return data;
}

// Функция для сохранения текстуры в PPM формат
MentalResult mental_save_ground_texture(const char* filename, int width, int height, float scale) {
    unsigned char* data = mental_generate_ground_texture(width, height, scale);
    if (!data) {
        return MENTAL_FAILED_TO_ALLOCATE_MEMORY;
    }
    
    FILE* file = fopen(filename, "wb");
    if (!file) {
        free(data);
        return MENTAL_FILE_OPEN_FAILED;
    }
    
    // Записываем заголовок PPM
    fprintf(file, "P6\n%d %d\n255\n", width, height);
    
    // Записываем данные
    fwrite(data, 1, width * height * 3, file);
    
    fclose(file);
    free(data);
    
    MENTAL_DEBUG("Ground texture saved to: %s", filename);
    return MENTAL_OK;
}
